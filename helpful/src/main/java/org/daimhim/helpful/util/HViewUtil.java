...
public class HViewUtil {
    ...
    public static float dip2px(Context context, float dipValue) {
        DisplayMetrics mDisplayMetrics = HAppUtil.getDisplayMetrics(context);
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		float scale = 1;
		try {
			float scaleWidth = (float) displayWidth / HAppConfig.UI_WIDTH;
			float scaleHeight = (float) displayHeight / HAppConfig.UI_HEIGHT;
	 ...
	 * 
	 * 描述：View树递归调用做适配.
	 * HAppConfig.uiWidth = 1080;
	 * HAppConfig.uiHeight = 700;
    ...
    public static void scaleContentView(ViewGroup contentView){
        HViewUtil.scaleView(contentView);
    ...
    */
    public static void setViewSize(View view,int widthPixels, int heightPixels){
        ...
        if(params == null){
            HLogUtil.e(HViewUtil.class, "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
