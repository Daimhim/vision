...
import org.daimhim.banner2.listener.OnBannerListener;
import org.daimhim.banner2.loader.ImageLoaderInterface;
...
public class Banner extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    private void initThreePages(View pView) {
        if (threePages){
            ((ViewGroup) pView.findViewById(R.id.bannerContainer)).setClipChildren(false);
    ...
    public void onPageSelected(int position) {
        ...
        switch (bannerStyle) {
            case BannerConfig.CIRCLE_INDICATOR:
                break;
            case BannerConfig.NUM_INDICATOR:
                numIndicator.setText(String.format("%s/%s",position , count));
                break;
            case BannerConfig.NUM_INDICATOR_TITLE:
                numIndicatorInside.setText(String.format("%s/%s",position , count));
